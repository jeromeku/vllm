...
from .registry import QWEN3_EXAMPLE_MODELS
...
def get_layer_prefixes_to_exclude(
    ...
    return [f"layers.{i}." for i in range(num_layers_to_load, num_layers)]
...
def patched_loader(model_id: str, num_layers_to_load: int = NUM_LAYERS_TO_LOAD):
    ...
    def load_weights(
        ...
        skip_substrs = layer_pats_to_exclude
        if self.config.tie_word_embeddings:
            skip_prefixes = ["lm_head."]
        logger.debug(
            f"Setting skip prefixes for {model_id} to {skip_prefixes}, substrs {skip_substrs}"
        )
        loader = AutoWeightsLoader(
            self, skip_prefixes=skip_prefixes, skip_substrs=skip_substrs
        )
...
def test_qwen3(model_arch: str, num_layers_to_load: int = 1):
    ...
    with ExitStack() as stack:
        ...
        # ignore_patterns = get_layer_prefixes_to_exclude(model_info.default)
        llm = LLM(
            model_info.default,
...
if __name__ == "__main__":
    vllm_env = "\n".join(
        f"{k}={v}" for k, v in os.environ.items() if "VLLM" in k
    )
    logger.debug(f"Running with VLLM env: {vllm_env}")
